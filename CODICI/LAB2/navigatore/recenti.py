...
def cerca_recenti(nome,datalimite,giaesplorati):
	"""resituisce la lista dei file modificati dopo datalimite
 tra quelli nella directory nome e sottodirectory"""
	
	assert os.path.isdir(nome), "Argomento deve essere una directory"
	print(f"Begin: {nome}",file=sys.stderr)
	# inizializzo la lista di output inizialmnte vuota
	recenti = []
	...
	for f in listafile:
		...
		if not os.path.isdir(nomecompleto):
			...
			recenti_ric = cerca_recenti(nomecompleto,datalimite, giaesplorati)
